...
class OverallDensity(object):
    ...
    #discretiza os clusters em "regiões de abordagem", retorna um dicionario com as pessoas (inclusive as virtuais) e os pontos de cada tipo
    def discretize_clusters_to_approach_regions(self, people):
        ...
        # Cria uma lista para armazenar pontos de interesse em cada dicionário para cada pessoa na cena
        for person in people:
        ...
        for cluster_idx, cluster in enumerate(connected_components):
            ...
            self.segments_samples[cluster_idx + 1] = {
                ...
                'persons': [],
                'virtual_persons': [],  # Adiciona uma lista para pessoas virtuais
                'prohibited_approach': {},
                'approach_segments': {},
            ...
            for person_idx in cluster:
                ...
                self.segments_samples[cluster_idx + 1]['persons'].append(person)
            ...
            if num_people == 1:
                ...
                # Adiciona os pontos ao dicionário segments_samples
                for approach_type, points_dict in [('prohibited_approach', points_in_fov), ('approach_segments', approach_points),
                                                   ('better_approach_segments', better_approach_points)]:
                    self.segments_samples[cluster_idx + 1][approach_type][person] = points_dict[person]
    
            elif num_people > 1:
                # calculo do xc e yc e lista pra guardar as pessoas identico pra n=2
                X, Y = np.meshgrid(self.x, self.y)
                ...
                for person1, person2 in zip(guard_persons, guard_persons[1:] + [guard_persons[0]]):
                    ...
                    for i in range(len(par_persons) - 1):
                        ...
                        # Adicione as pessoas virtuais ao dicionário
                        self.segments_samples[cluster_idx + 1]['virtual_persons'].extend([person_Ocenter, person_Ocenter2])
    
                        for row in range(self.cluster[cluster_idx].shape[0]):
                            for col in range(self.cluster[cluster_idx].shape[1]):
                                if self.cluster[cluster_idx][row, col] == 1:
                                    point_x = self.x[col]
                                    point_y = self.y[row]
        ...
        #print(self.segments_samples)
        #return points_in_fov, approach_points, better_approach_points, self.segments_samples
        return self.segments_samples
    ...
    def draw_overall(self, drawDensity=False, drawCluster=False, drawGraph=False, drawSegment=False, people=None, plot=plt):
        ...
        if drawGraph:
            ...
            nx.draw_networkx(self.G, pos, node_size=0, with_labels=False)
        ...
        if drawSegment and people is not None:
            # Plote os pontos coletados no dicionário segments_samples
            for cluster_idx, cluster_data in self.segments_samples.items():
                for approach_type in ['prohibited_approach', 'approach_segments', 'better_approach_segments']:
                    for person, points in cluster_data[approach_type].items():
                        if len(points) > 0:
                            xs, ys = zip(*points)
                            if approach_type == 'prohibited_approach':
                                plot.scatter(xs, ys, marker='o', color='red')
                            elif approach_type == 'approach_segments':
                                plot.scatter(xs, ys, marker='o', color='yellow')
                            elif approach_type == 'better_approach_segments':
                                plot.scatter(xs, ys, marker='o', color='green')
