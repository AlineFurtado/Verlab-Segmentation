...
'''Classifica segmentos de approach, atribui profits e seleciona amostras centrais em cada segmento'''
...
class OverallDensity(object):
    ...
    def __init__(self, person=None, zone=None, map_resolution=100, window_size=1):
        ...
        self.better_approach_points = {}
        self.euclidean_distance = lambda p1, p2: np.sqrt((p1[0] - p2[0])**2 + (p1[1] - p2[1])**2)
        self.calculate_median = lambda points: tuple(np.median(points, axis=0))
        self.calculate_profit = lambda segment, num_people, cost, benefit, color: benefit - cost  # Simplificação
        self.segments_samples = {}
    ...
    def to_networkx_graph(self):
        """
        Converte o grafo OverallDensity em um objeto de grafo do NetworkX.
        """
        G_networkx = nx.Graph()
        ...
        for node in self.G.nodes():
            ...
            G_networkx.add_node(node, pos=pos)

        # Adicione arestas ao grafo
        for edge in self.G.edges():
            weight = self.G.edges[edge]['weight']
            G_networkx.add_edge(edge[0], edge[1], weight=weight)
    ...
    def calculate_median(self, segment_points):
        if not segment_points:
            return None

        segment_length = len(segment_points)
        central_index = segment_length // 2

        if segment_length % 2 == 0:
            central_point = random.choice([segment_points[central_index - 1], segment_points[central_index]])
        else:
            central_point = segment_points[central_index]

        return central_point

    def calculate_profit(self, segment_points, num_people, min_profit, max_profit, last_color):
    ...
    def get_color(self, point, points_in_fov, better_approach_points, approach_points):
        for person, points in better_approach_points.items():
            if point in points:
                return 'green'
        for person, points in approach_points.items():
            if point in points:
                return 'yellow'
        return 'red'
    ...
    #processa cada segmento, extraindo o ponto central como amostra e atribuindo um profit
    def _process_segments(self, cluster_idx, persons, num_people, id_segment, virtual_persons=None):
        for person in persons:
            if num_people == 1:
                # Caso com 1 pessoa, manter lógica existente
                if (np.deg2rad(-30) <= person.th <= np.deg2rad(30)) or (person.th == np.deg2rad(180)):
                    last_approach_point_before_better = None
                    first_approach_point_after_better = None
    
                    if self.better_approach_points[person]:
                        last_approach_point_before_better = self.approach_points[person][-1]
                        first_approach_point_after_better = self.approach_points[person][0]
                else:
                    last_approach_point_before_better = None
                    first_approach_point_after_better = None
    
                    sorted_approach_points = sorted(self.approach_points[person])
                    sorted_better_approach_points = sorted(self.better_approach_points[person])
    
                    for point in sorted_approach_points:
                        if point < sorted_better_approach_points[0]:
                            last_approach_point_before_better = point
                        else:
                            break
    
                    for point in sorted_approach_points:
                        if point > sorted_better_approach_points[-1]:
                            first_approach_point_after_better = point
                            break
    
                if last_approach_point_before_better and first_approach_point_after_better:
                    threshold_distance = self.euclidean_distance(last_approach_point_before_better, first_approach_point_after_better)
    
                    for approach_type, points in [('approach_segments', self.approach_points[person]),
                                                  ('better_approach_segments', self.better_approach_points[person])]:
                        sorted_points = sorted(points)
                        if sorted_points:
                            current_segment = [sorted_points[0]]
                            for prev, curr in zip(sorted_points[:-1], sorted_points[1:]):
                                if self.euclidean_distance(prev, curr) >= threshold_distance:
                                    segment_size = len(current_segment)
                                    median_point = self.calculate_median(current_segment)
                                    last_color = 'yellow' if approach_type == 'approach_segments' else 'green'
                                    profit = self.calculate_profit(current_segment, num_people, 50, 100, last_color)
                                    self.segments_samples[cluster_idx + 1][approach_type].append({
                ...
                # Caso com mais de 1 pessoa, usar virtual_persons para verificar os ângulos
                for virtual_person in virtual_persons:
                    if (np.deg2rad(-30) <= virtual_person.th <= np.deg2rad(30)) or (virtual_person.th == np.deg2rad(180)):
                        last_approach_point_before_better = None
                        first_approach_point_after_better = None
    ...
    def discretize_clusters_to_approach_regions(self, people):
        # Cria dicionários específicos para armazenar pontos de interesse
        self.points_in_fov = {}
        self.approach_points = {}
        self.better_approach_points = {}
        ...
        # Obter uma lista de componentes conectados
        connected_components = list(nx.connected_components(G))
        
        # Criar uma lista para armazenar pontos de interesse em cada dicionário para cada pessoa na cena
        for person in people:
            self.points_in_fov[person] = []
            self.approach_points[person] = []
            self.better_approach_points[person] = []
        
        # Dicionário para amostragem dos segmentos classificados
        #segments_samples = {}
        id_segment = 1  # Contador de segmentos único
        ...
        for cluster_idx, cluster in enumerate(connected_components):
            ...
            self.segments_samples[cluster_idx + 1] = {
                'num_people': num_people,
                'approach_segments': [],
                'better_approach_segments': []
            }
            if num_people == 1:
                    ...
                    for col in range(self.cluster[cluster_idx].shape[1]):
                        if self.cluster[cluster_idx][row, col] == 1:  # Verifique se o ponto pertence ao cluster
                            ...
                            if self.is_point_in_field_of_vision(person, point_x, point_y):
                                self.points_in_fov[person].append((point_x, point_y))
                            
                            # Verifique se o ponto está na região de approach da pessoa
                            if self.is_approach_point(person, point_x, point_y):
                                self.points_in_fov[person].remove((point_x, point_y))
                                self.approach_points[person].append((point_x, point_y))
                            
                            # Verifique se o ponto está na melhor região de approach da pessoa
                            if self.is_better_approach_point(person, point_x, point_y):
                                self.approach_points[person].remove((point_x, point_y))
                                self.better_approach_points[person].append((point_x, point_y))
                ...
                for row in range(self.cluster[cluster_idx].shape[0]):
                    for col in range(self.cluster[cluster_idx].shape[1]):
                        if self.cluster[cluster_idx][row, col] == 1:  # Verifique se o ponto pertence ao cluster
                            ...
                            # Verifique se o ponto está nas costas da pessoa
                            if self.is_point_back2(persons, point):
                                # Classifica esse ponto na região NAR
                                self.points_in_fov[person].append(point)

                # Processar os segmentos
                self._process_segments(cluster_idx, persons, num_people, id_segment)
                ...
                for person1, person2 in zip(guard_persons, guard_persons[1:] + [guard_persons[0]]):
                    ...
                    for i in range(len(par_persons) - 1):
                        ...
                        virtual_persons.append(person_Ocenter)
                        person_Ocenter2 = Person(x_med, y_med, th_opost, 1)
                        virtual_persons.append(person_Ocenter2)
                ...
                for person in guard_persons:
                    ...
                    #with open('saida.txt', 'a') as arquivo:
                    for point in self.better_approach_points[person]:
                    ...
                    for point in self.approach_points[person]:
                        #print("ponto na lista approach",file=arquivo)
                        if any(self.is_point_back(guard_persons, point) for p in guard_persons if p != person):
                            points_to_move_approach.append(point)
                    self.better_approach_points[person] = [point for point in self.better_approach_points[person] if point not in points_to_move_better]
                    self.approach_points[person] = [point for point in self.approach_points[person] if point not in points_to_move_approach]
                    self.points_in_fov[person].extend(points_to_move_better)
                    self.points_in_fov[person].extend(points_to_move_approach)

                # Processar os segmentos
                #print(f'guard_persons:{guard_persons}')
                #print(f'virtual_persons:{virtual_persons}')
                self._process_segments(cluster_idx, guard_persons, num_people, id_segment, virtual_persons)
        
        #print(f'better_approach_points[person]')
        #print(f'approach_points:{len(self.approach_points[person])}, better:{len(self.better_approach_points[person])}')        
        return self.points_in_fov, self.approach_points, self.better_approach_points, self.segments_samples
    ...
    def draw_overall(self, drawDensity=False, drawCluster=False, drawGraph=False, drawSegment=False, people=None, plot=plt):
        ...
        if drawSegment and people is not None:
            ...
            segment_points_dict, approach_points_dict, better_approach_points_dict, segments_samples = self.discretize_clusters_to_approach_regions(people)
            ...
            for person, points in segment_points_dict.items():
                if len(points) > 0:
                    xs, ys = zip(*points)
		    #print(f'numero de pontos vermelhos:{len(xs)}')
                    plot.scatter(xs, ys, marker='o', color='red')
            for person, points in approach_points_dict.items():
                if len(points) > 0:
                    xb, yb = zip(*points)
		    #print(f'numero de amarelos:{len(xs)}')
                    plot.scatter(xb, yb, marker='o', color='yellow')
            for person, points in better_approach_points_dict.items():
                if len(points) > 0:
                    xb, yb = zip(*points)
                    #print(f'numero de pontos verdes:{len(xs)}')
                    plot.scatter(xb, yb, marker='o', color='green')
            coordinates = []
            # Itera sobre os clusters no dicionário segments_samples
            for cluster_key, cluster_info in segments_samples.items():
                # Verifica se cluster_info é um dicionário
                if isinstance(cluster_info, dict):
                    # Itera sobre os tipos de segmentos no dicionário cluster_info
                    for segment_type, segment_list in cluster_info.items():
                        # Verifica se segment_list é realmente uma lista
                        if isinstance(segment_list, list):
                            # Itera sobre cada segmento na lista
                            for segment in segment_list:
                                # Adiciona as coordenadas do ponto mediano às coordenadas se elas existirem
                                if 'median_point' in segment:
                                    coordinates.append(segment['median_point'])
    
            # Se coordenadas não estiverem vazias, plote os pontos
            if coordinates:
                x_vals = [coord[0] for coord in coordinates]
                y_vals = [coord[1] for coord in coordinates]
                plot.scatter(x_vals, y_vals, color='blue')  # Pontos azuis
